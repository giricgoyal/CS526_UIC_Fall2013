...
def addCrimeData():
	...
	for rawData in manageData.getCrimeData():
		if (rawData[0] != None and rawData[1] != None and rawData[2] != None):
			#print rawData
			lon = rawData[0]
			lat = rawData[1]
...
def addCommunityNames():
	for data in manageData.getCommunityLocations():
		...
		t = Text3D.create('fonts/Verdana.ttf', 150, str(name))
...
def addCommunityBoundaries(show,countList):
	...
	for data in dataList:
		...
		line = LineSet.create()
		t = Text3D.create('fonts/Verdana.ttf', 150, "")
		if show == True:
			t = Text3D.create('fonts/Verdana.ttf', 150, "\n"+str(value))
...
def jumpToCommunity():
	...
	for data in manageData.getCommunityLocationByName():
		...
		print position
		cameraManager.jumpToComm(Vector3(float(position[0]), float(position[1]), 8000))
